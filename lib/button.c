...
static void handle_button_press_sequence(uint16_t period, uint16_t timer_seconds, bool *error_occurred, bool *test_mode) {
...
void handle_button_timer_interrupt(uint16_t period, uint16_t timer_seconds, bool *error_occurred, bool *test_mode) {
    if (button_wait_timer > 0) {
        ...
        handle_button_press_sequence(period, timer_seconds, error_occurred, test_mode);
    }
}
